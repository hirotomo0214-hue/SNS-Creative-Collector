...
def collect_links(page, search_url: str, max_scrolls: int = 8) -> list[str]:
    """Keep Instagram discovery order instead of converting results to a set."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--anchor-keywords",
        default="",
        help="Comma-separated high-confidence keywords. If set, at least one must appear in the post text.",
    )
    ...
    with sync_playwright() as p:
        ...
        for keyword, search_url in search_sources:
            ...
            if links is None:
                ...
                continue
            for url in links:
                source_list = candidate_sources.setdefault(url, [])
        ...
        for candidate_index, url in enumerate(candidate_urls, start=1):
            ...
            item["matched_keywords"] = matched_terms(description, keywords)
            item["matched_anchor_keywords"] = matched_terms(description, anchors)
            item["discovered_by_keywords"] = candidate_sources.get(url, [])
            item["japanese_text"] = bool(JAPANESE_RE.search(description))

            if anchors and not item["matched_anchor_keywords"]:
                skip_reasons["irrelevant_recent"] += 1
                continue
            if not anchors and not item["matched_keywords"]:
                skip_reasons["irrelevant_recent"] += 1
                continue
            if args.require_japanese and not item["japanese_text"]:
                skip_reasons["non_japanese"] += 1
                continue

            item["relevance_reason"] = (
                "anchor_keyword_match" if anchors else "discovery_keyword_match"
            )
    ...
    print(f"Recent candidates within {args.days} days: {recent_candidate_count}")
