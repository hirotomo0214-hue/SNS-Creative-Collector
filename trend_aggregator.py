...
INSTAGRAM_CODE_RE = re.compile(r"instagram\.com/(?:p|reel)/([^/?#]+)", re.I)
...
def completeness(row):
    score = 0
    for key in ["構成案", "具体的な構成案", "訴求方法", "案件ジャンル", "媒体", "投稿形態"]:
        value = row.get(key)
...
def signal_tokens(row):
    ...
    for value in row.get("訴求方法_list", []):
        tokens.append(("appeal", value))
    for value in row.get("案件ジャンル_list", []):
        tokens.append(("genre", value))
    if row.get("投稿形態"):
...
def score_signal(cur, prev, current_days, baseline_days):
    ...
    if cur_posts >= 3 and cur_accounts >= 3 and lift >= 1.3:
        status = "trend"
    ...
    concentration = "cross_account" if cur_accounts >= 3 else "campaign_concentrated"
    return score, round(lift, 2), status, concentration


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--input", required=True, help="JSON file containing either a list of Notion rows or {'results': [...]} payload")
    ap.add_argument("--output", default="trend_report.json")
    ap.add_argument("--as-of", default=None, help="YYYY-MM-DD; defaults to UTC today")
    ...
    for key, cur in current_stats.items():
        ...
        score, lift, status, concentration = score_signal(cur, prev, args.current_days, args.baseline_days)
        ...
        signals.append({
            ...
            "status": status,
            "trend_score": score,
            "current_posts": cur["posts"],
            "current_accounts": len(cur["accounts"]),
            "current_projects": len(cur["projects"]),
            "baseline_posts": prev["posts"],
            "rate_lift": lift,
    ...
    missing_classification = [
        {"name": r.get("名前"), "url": r.get("動画URL"), "posted_at": str(r.get("posted_date") or "")}
        for r in current_rows
        if not r.get("構成案") and not r.get("訴求方法_list")
    ]
    ...
    payload = {
        ...
        "as_of": str(as_of),
        "windows": {
            "current": {"start": str(current_start), "end": str(as_of), "days": args.current_days},
            "baseline": {"start": str(baseline_start), "end": str(baseline_end), "days": args.baseline_days},
        },
        "input_rows": len(rows),
        "normalized_unique_rows": len(normalized),
        "duplicate_rows_removed": len(duplicates),
        "duplicates": duplicates,
        "current_rows": len(current_rows),
        "current_usable_rows": current_usable,
        "baseline_rows": len(baseline_rows),
        "baseline_usable_rows": baseline_usable,
        "missing_classification_count": len(missing_classification),
        "missing_classification": missing_classification,
        "signals": signals,
        "top_structures": [s for s in signals if s["kind"] == "structure"][:10],
        "top_appeals": [s for s in signals if s["kind"] == "appeal"][:10],
        "top_formats": [s for s in signals if s["kind"] == "format"][:10],
        "top_genres": [s for s in signals if s["kind"] == "genre"][:10],
    }
    ...
    for s in signals[:10]:
        print(f"- {s['status']} {s['kind']}:{s['name']} posts={s['current_posts']} accounts={s['current_accounts']} lift={s['rate_lift']} score={s['trend_score']}")
